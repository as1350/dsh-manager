...
window.__ModuleLoader__.load({ id: '@deepseek-ai/dsh-manager', factory: (require) => {
  ...
  const zh = {
    ...
    'group.global': '全局', 'group.global.hint': '你自己的通用技能（~/.dsh/skills）', 'group.project': '项目', 'group.project.hint': '当前项目目录中的技能', 'group.preset': '插件 / 预设', 'group.preset.hint': '由插件或 Agent 预设动态注入', 'group.builtin': '系统内置', 'group.builtin.hint': '代码内嵌或随内置 provider 提供', 'group.empty': '此来源暂无技能', 'scope.global': '全局', 'scope.preset': '{name} 预设', 'card.crossScope': '来自其他预设作用域 · 仅查看', 'search.results': '匹配 {count} 个技能', 'search.empty': '没有找到匹配的技能',
  ...
  const en = {
    ...
    'group.global': 'Global', 'group.global.hint': 'Your own general-purpose skills (~/.dsh/skills)', 'group.project': 'Project', 'group.project.hint': 'Skills in the current project directory', 'group.preset': 'Plugin / Preset', 'group.preset.hint': 'Injected by plugins or agent presets', 'group.builtin': 'Built-in', 'group.builtin.hint': 'Embedded in code or shipped with a provider', 'group.empty': 'No skills from this source', 'scope.global': 'Global', 'scope.preset': '{name} preset', 'card.crossScope': 'From another preset scope · read-only', 'search.results': '{count} matching skills', 'search.empty': 'No matching skills',
    ...
    return h('article', { className: 'skm-card' },
      h('div', { className: 'skm-card-top' },
        h('div', { className: 'skm-card-head' },
          ...
          h('span', { className: 'skm-card-scope', title: crossScope ? t('card.crossScope') : (props.scopeLabel === 'global' ? t('scope.global') : t('scope.preset', { name: props.scopeLabel })) }, props.scopeLabel === 'global' ? t('scope.global') : t('scope.preset', { name: props.scopeLabel })),
