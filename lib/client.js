...
window.__ModuleLoader__.load({ id: '@deepseek-ai/dsh-manager', factory: (require) => {
  ...
  function rpc(method, args) {
    ...
    const timer = controller !== null ? window.setTimeout(function () { controller.abort() }, 30000) : null
  ...
  function RepoPanel(props) {
    ...
    const pollAiStatus = function (hash) {
      port.aiExplainStatus({ hash: hash }).then(function (res) {
        ...
        if (st.state === 'done') {
          ...
          setAiEntryStates(function (prev) { const next = Object.assign({}, prev); next[hash] = { state: st.state === 'queued' ? 'queued' : 'running' }; return next })
          setAiModal(function (m) { return m === null || m.hash !== hash ? m : Object.assign({}, m, { state: st.state === 'queued' ? 'queued' : 'running', text: typeof st.text === 'string' ? st.text : '', error: typeof st.error === 'string' ? st.error : '' }) })
        }
      }).catch(function () {})
    }
    ...
    React.useEffect(function () {
      if (open !== true) { setDetail(null); setReadmeExpanded(false) }
    }, [open])
      ...
      return h('div', { key: key, className: 'skm-svc-row ' + statusClass },
        ...
        h('div', { className: 'skm-svc-actions' },
          ...
          h('span', { className: 'skm-menu-wrap' },
            ...
            rowMenuOpen ? h('div', { className: 'skm-menu' },
