...
window.__ModuleLoader__.load({ id: '@deepseek-ai/dsh-manager', factory: (require) => {
  ...
  const en = {
    ...
    'group.global': 'Global', 'group.global.hint': 'Your own general-purpose skills (~/.dsh/skills)', 'group.project': 'Project', 'group.project.hint': 'Skills in the current project directory', 'group.plugin': 'Plugins', 'group.plugin.hint': 'Injected by host plugins', 'group.preset': 'Presets', 'group.preset.hint': 'Injected by agent preset scopes', 'group.builtin': 'Built-in', 'group.builtin.hint': 'Embedded in code or shipped with a provider', 'group.empty': 'No skills from this source', 'scope.global': 'Global', 'scope.project': 'Project', 'scope.plugin': 'Plugin', 'scope.preset': '{name} preset', 'card.crossScope': 'From another preset scope · read-only', 'card.preview': 'Not mounted · open a session to activate', 'lock.locked': 'Locked · click to unlock and toggle invocation', 'lock.unlocked': 'Unlocked · click to lock again', 'search.results': '{count} matching skills', 'search.empty': 'No matching skills',
