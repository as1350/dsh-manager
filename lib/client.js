...
window.__ModuleLoader__.load({ id: '@deepseek-ai/dsh-manager', factory: (require) => {
  ...
  function RepoPanel(props) {
    ...
    const draftUpdateMirror = function (mirror) {
      ...
      draft('请更新镜像 ' + mirror.name + ' 并处理装配检查：\n' +
        '1. cd ' + mirror.path + '\n' +
        '2. 记录当前 HEAD（旧版本）；git fetch upstream\n' +
        '3. git reset --hard upstream/' + branch + '\n' +
        '4. 若存在 submodule：git submodule update --init --recursive\n' +
        '5. 更新 repos.json 中该项目的 lastSync/updatedAt；更新 REPOS.md\n' +
        '6. 判断该项目是否为已装配插件：读 D:\\Desktop\\Dsh\\本地项目\\_governance\\MANIFEST.md（装配资产清单）与 C:\\Users\\ThinkBook\\.dsh\\profiles\\web\\package.json（dependencies 与 dsh.profile.bundles）是否含该包，dev_plugin_status 查状态\n' +
        '7. 若已装配且版本有变化：\n' +
        '   a. 总结更新内容：git log 旧HEAD..新HEAD（提交数/涉及文件/主要变更点）\n' +
        '   b. 按 local-governance 流程 A/B 更新装配：构建（npm install && npm run build）→ npm pack 快照入 _snapshots\\<包名>\\ → profile 依赖切 file: 新 tgz → 清 node_modules\\.pnpm\\lock.yaml + .package-map.json 后 pnpm install → 官方 bundle 通道需重启 DSH，loader 管理模块用 dev_reload_package → 更新 MANIFEST.md（装配版本/归档版本/备注）\n' +
        '8. 按 local-governance 流程 I：镜像同步静默入队 pending-reviews.json；若发生装配升级，完成后询问用户立即审核或推迟\n' +
        '9. 汇报：镜像更新结果（旧→新版本/commit）+ 更新内容总结 + 装配状态（已装配已更新 / 版本未变 / 未装配仅镜像同步）')
    }
    ...
    const detailOverlay = detail !== null && detail.loading !== true ? h('div', { className: 'skm-detail-backdrop', onClick: function (event) { if (event.target === event.currentTarget) setDetail(null) } },
    ...
    const aiOverlay = aiModal !== null ? h('div', { className: 'skm-detail-backdrop skm-ai-backdrop', onClick: function (event) { if (event.target === event.currentTarget) { clearAiPoll(); setAiModal(null) } } },
  ...
  function ServicePanel(props) {
    ...
    const draftService = function (path) {
      const text = '请用 service-config 为项目 ' + path + ' 生成服务配置：识别服务类型（Node / Python venv / 前后端分离 / 多服务），生成全部服务的 command/args/env/port/healthUrl 静默配置草稿供确认，写入 services.json 后自动阶梯测试（进程存活 → 端口 → healthUrl），测完停止。'
      const res = writeDraft(currentId, text)
      if (res.ok === true) {
    ...
    const openEditor = function (projectPath, index, svc) {
      if (index === -1) {
        setEditor({ projectPath: projectPath, index: -1, draft: { name: '', cwd: projectPath, command: '', args: '', env: '', port: '', autoStart: false, autoRestart: false, detached: false, healthUrl: '', envFile: '', startTimeoutMs: '', note: '' } })
    ...
    const currentProjectServices = function (projectPath) {
      ...
      return project.services.map(function (s) { return { name: s.name, cwd: s.cwd, command: s.command, args: s.args, env: s.env, port: s.port, autoStart: s.autoStart === true, autoRestart: s.autoRestart === true, detached: s.detached === true, healthUrl: typeof s.healthUrl === 'string' ? s.healthUrl : null, envFile: typeof s.envFile === 'string' ? s.envFile : '', startTimeoutMs: Number.isInteger(s.startTimeoutMs) ? s.startTimeoutMs : 30000, note: typeof s.note === 'string' ? s.note : '' } })
    }
    ...
    const serviceRow = function (projectPath, svc, index) {
      const key = projectPath + '|' + svc.name
      ...
      return h('div', { key: key, className: 'skm-svc-row ' + statusClass },
        ...
        h('div', { className: 'skm-svc-actions' },
          svc.port !== null && svc.port !== undefined
            ? h('button', { type: 'button', className: 'skm-btn', disabled: !running, title: 'http://127.0.0.1:' + svc.port, onClick: function () { window.open('http://127.0.0.1:' + svc.port, '_blank', 'noopener') } }, t('service.open'))
            : null,
          svc.state === 'external'
          ...
          h('span', { className: 'skm-menu-wrap', ref: rowMenuOpen ? menuWrapRef : null },
            ...
            rowMenuOpen ? h('span', { className: 'skm-menu-backdrop', onClick: function () { setRowMenu(null) } }) : null,
    ...
    if (projects.length === 0) {
      ...
      if (activeProjects.length > 0) {
        contentChildren.push(h('div', { className: 'skm-repo-list', key: 'list' }, activeProjects.map(function (p) {
          ...
          let projMenuEl = null
          if (menuOpen) {
            projBackdropEl = h('span', { className: 'skm-menu-backdrop', onClick: function () { setProjMenu(null) } })
          ...
          return h('div', { key: p.path, className: 'skm-svc-project' },
            projHeadEl,
            h('div', { className: 'skm-svc-rows' }, configured.map(function (row) { return serviceRow(p.path, row.svc, row.index) })))
        })))
      }
    ...
    const aiSettingsOverlay = aiSettingsOpen === true ? (function () {
      ...
      const aiEffortOptions = aiEffortList.map(function (e) { return e.id })
      if (aiEffortOptions.indexOf(aiEffortValue) === -1) aiEffortOptions.unshift(aiEffortValue)
      return h('div', { className: 'skm-cfg-backdrop', onClick: function (event) { if (event.target === event.currentTarget) setAiSettingsOpen(false) } },
    ...
    const editorOverlay = editor !== null ? h('div', { className: 'skm-cfg-backdrop', onClick: function (event) { if (event.target === event.currentTarget) setEditor(null) } },
    ...
    const logOverlay = logViewer !== null ? h('div', { className: 'skm-cfg-backdrop', onClick: function (event) { if (event.target === event.currentTarget) setLogViewer(null) } },
    ...
    const extModalOverlay = extModal !== null ? h('div', { className: 'skm-cfg-backdrop', onClick: function (event) { if (event.target === event.currentTarget) setExtModal(null) } },
    ...
    const killConfirmOverlay = killConfirm !== null ? h('div', { className: 'skm-cfg-backdrop', onClick: function (event) { if (event.target === event.currentTarget) setKillConfirm(null) } },
    ...
    return h(React.Fragment, null,
      h('div', { className: 'skm-backdrop', onClick: function (event) { if (event.target === event.currentTarget) serviceOpenStore.set(false) } },
