...
import { readFileSync, createWriteStream, openSync, closeSync } from 'node:fs'
...
function registerPackagedSkills(ctx) {
  const PACKAGED = [
    ...
    ['service-config', '../skills/service-config/SKILL.md'],
  ]
...
async function computeServiceStatus(path, entry, info) {
  ...
  if (running) {
    ...
    if (hu !== null) {
      healthUrl = hu
      const full = /^https?:\/\//i.test(hu) ? hu : 'http://127.0.0.1:' + (entry.port || 80) + (hu[0] === '/' ? hu : '/' + hu)
      healthUp = await probeHttpUrl(full, 1500)
    }
  }
  let state = 'stopped'
  let external = false
...
async function doStartService(settings, pathKey, name) {
  ...
  if (existing !== null && existing !== undefined && isPidAlive(Number(existing.pid))) {
    return { ok: true, alreadyRunning: true, status: await computeServiceStatus(pathKey, entry, existing) }
  }
...
async function refreshRepoDetailInBackground(path, settings) {
  ...
  try {
    ...
    repoDetailCache.set(path, { ts: Date.now(), data: fresh, fp: fp })
...
export function apply(ctx) {
  ...
  const handlers = {
    ...
    'repoScan': async function () {
      ...
      const detectedAll = await Promise.all(foundAll.map(function (item) {
        ...
        if (meta !== null && typeof meta.cloudRepo === 'string' && meta.cloudRepo.length > 0 && typeof meta.private === 'boolean') {
          return Promise.resolve({ cloudRepo: meta.cloudRepo, private: meta.private })
        }
        return detectRepoMeta(item.p.path)
      }))
      const projects = []
      const mirrors = []
    ...
    'repoDetail': async function (args) {
      ...
      if (entry !== undefined && entry !== null && entry.fp !== null && entry.fp !== undefined && fp.isRepo === true && repoFingerprintCoreEqual(fp, entry.fp)) {
        ...
        repoDetailCache.set(path, { ts: now, data: data, fp: entry.fp })
        return data
      }
      // 全量构建
      const data = await buildRepoDetail(path)
      const coreFp = repoFingerprintCore(await computeRepoFingerprint(path))
      repoDetailCache.set(path, { ts: Date.now(), data: data, fp: coreFp })
    ...
    'serviceAiDiagnose': async function (args) {
      ...
      const prompt = [
        '项目路径：' + entry.cwd,
        '启动命令：' + entry.command + (Array.isArray(entry.args) && entry.args.length > 0 ? ' ' + entry.args.join(' ') : ''),
    ...
    'serviceAiDraft': async function (args) {
      ...
      let envExample = ''
      for (const n of ['.env.example', '.env']) {
        try { const t = await readFile(join(rawPath, n), 'utf8'); if (typeof t === 'string' && t.trim().length > 0) { envExample = t.slice(0, 3000); break } } catch (error) {}
      }
      const parts = []
      if (pkg !== null) parts.push('package.json：' + JSON.stringify(pkg))
      if (readme !== null) parts.push('README（截取）：\n' + (readme.text || '').slice(0, 6000))
      if (envExample.length > 0) parts.push('.env.example/.env（截取，注意不要把真实密钥写进 env 建议值）：\n' + envExample)
        ...
        return readServicesState().then(function (state) {
          ...
          return Promise.all(Object.keys(state.services).map(async function (key) {
            ...
            if (entry !== undefined) {
              ...
              if (related === false) { delete state.services[key]; return }
            }
