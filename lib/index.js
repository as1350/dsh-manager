...
import { readFile, writeFile, rename, unlink, rmdir, readdir, mkdir, access, rm, stat, cp } from 'node:fs/promises'
...
function registerPackagedSkills(ctx) {
  ...
  for (const [skillName, relPath] of PACKAGED) {
    let url = null
    try {
      url = new URL(relPath, import.meta.url)
      ...
      console.warn(`[dsh-manager] packaged skill "${skillName}" registration skipped (${url === null ? relPath : url.href}): ${error instanceof Error ? error.message : String(error)}`)
    }
  }
}
...
function resolveCustomSkillDir(expr, presetDir) {
  ...
  if (expr.indexOf('baseUrl') !== -1 || expr.indexOf('new URL') !== -1) {
    ...
    const value = runInNewContext(expr, sandbox, { timeout: 1000 })
...
const REPO_EXCLUDE_DIRS = new Set(['_governance', '_snapshots', 'skill仓库'])
...
async function readRepoSettings() {
  const defaults = { roots: [], governanceRoot: '', rootTypes: {} }
  try {
    ...
    return { roots: roots, governanceRoot: governanceRoot, rootTypes: rootTypes }
  } catch (error) {
    return defaults
  }
}
...
async function writeRepoSettings(settings) {
  ...
  const next = { roots: roots, governanceRoot: governanceRoot, rootTypes: rootTypes }
...
async function scanProjectDirs(root) {
  ...
  async function walk(current, relParts) {
    ...
    for (const ent of entries) {
      ...
      if (hasGit || hasPkg || hasDsh) {
        out.push({ name: name, path: dir, rel: rel.join('/'), hasGit: hasGit, hasPkg: hasPkg, hasDsh: hasDsh })
        continue
      }
      await walk(dir, rel)
    }
...
function cloudRepoFromRemote(url) {
  let s = String(url || '').trim().replace(/\.git$/, '')
...
const REPO_META_TTL = 60000

function clearRepoMetaCache(path) {
  repoMetaCache.delete(path)
}

async function detectRepoMeta(path) {
  const cached = repoMetaCache.get(path)
  if (cached !== undefined && Date.now() - cached.at < REPO_META_TTL) return cached.meta
  let meta = null
  try {
  ...
  repoMetaCache.set(path, { at: Date.now(), meta: meta })
  return meta
}
...
export function apply(ctx) {
  ...
  const handlers = {
    ...
    'setInvocation': async function (args) {
      ...
      const definition = await view.skills.get(name, { cwd: view.cwd, scope: view.scope })
      if (definition === undefined) return { error: '未找到技能 "' + name + '"' }
      if (typeof definition.path !== 'string' || definition.path.length === 0) return { error: '该技能无配置文件，无法修改调用权限' }
      const input = args !== null && typeof args === 'object' ? args : {}
    ...
    'repoSettingsSet': async function (args) {
      const input = args !== null && typeof args === 'object' ? args : {}
      const settings = await writeRepoSettings({ roots: input.roots, governanceRoot: input.governanceRoot, rootTypes: input.rootTypes })
      return { ok: true, settings: settings }
    },

    'repoScan': async function () {
      const settings = await readRepoSettings()
      ...
      for (const root of settings.roots) {
        ...
        for (const p of found) {
          ...
          const metaType = meta !== null && (meta.type === 'local' || meta.type === 'mirror') ? meta.type : null
          const rootType = settings.rootTypes[root] || (root === settings.governanceRoot ? 'local' : 'mirror')
          const detected = await detectRepoMeta(p.path)
          const cloudRepo = meta !== null && typeof meta.cloudRepo === 'string' && meta.cloudRepo.length > 0 ? meta.cloudRepo : (detected !== null ? detected.cloudRepo : '')
          const privateRepo = meta !== null && typeof meta.private === 'boolean' ? meta.private : (detected !== null ? detected.private : false)
          const row = { root: root, name: p.name, path: p.path, rel: typeof p.rel === 'string' ? p.rel : '', hasGit: p.hasGit, hasPkg: p.hasPkg, hasDsh: p.hasDsh, type: metaType || rootType, cloudRepo: cloudRepo, private: privateRepo, meta: meta }
          projects.push(row)
          if (row.type === 'mirror') mirrors.push(row)
        }
      }
      const skillGroups = await scanSkillMirror(settings.governanceRoot)
      return { ok: true, settings: settings, reposJson: reposJson, projects: projects, mirrors: mirrors, skillGroups: skillGroups }
    },
    ...
    'repoGitStates': async function (args) {
      ...
      const states = {}
      for (const p of paths) states[p] = await gitState(p)
    ...
    'repoSetVisibility': async function (args) {
      ...
      if (path.length > 0) clearRepoMetaCache(path)
      return { ok: true, private: visibility === 'private', cloudRepo: cloudRepo }
    },
    ...
    'repoDeleteSkill': async function (args) {
      ...
      const nextLines = lines.filter(function (line) {
        if (!line.includes('|') || line.trim().startsWith('| 技能名')) return true
        const cols = line.split('|').map(function (c) { return c.trim() })
        return cols[3] !== relBack
      })
      await atomicWrite(skillsPath, nextLines.join('\n'))
      return { ok: true }
    },
    ...
    'repoListDirs': async function (args) {
      ...
      for (const e of entries) {
        ...
        let isProject = false
        try { await access(join(dir, '.git')); isProject = true } catch (error) {}
        if (!isProject) { try { await access(join(dir, 'package.json')); isProject = true } catch (error) {} }
        if (!isProject) { try { await access(join(dir, '.dsh')); isProject = true } catch (error) {} }
        if (!isProject) dirs.push(e.name)
      }
    ...
    'repoCopySkillToGlobal': async function (args) {
      ...
      return copySkillDir(src, join(dshHome(), 'skills', name))
    },
    ...
    'repoCopySkillToProject': async function (args) {
      ...
      return copySkillDir(src, join(cwd, '.dsh', 'skills', name))
    },
  }
