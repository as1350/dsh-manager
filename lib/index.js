  ...
  for (const dirName of dirs) {
    ...
    try {
      ...
      ctx.effect(() => svc.register({
        ...
        whenToUse: typeof meta.whenToUse === 'string' && meta.whenToUse.length > 0 ? meta.whenToUse : undefined,
        invocation: invocation,
        source: 'custom',
        provider: 'dsh-manager',
        content: split.body.join('\n'),
        path: fileURLToPath(url),
        resourceBase: { kind: 'directory', path: fileURLToPath(new URL('./' + dirName + '/', root)) },
