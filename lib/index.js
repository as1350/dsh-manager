...
export function apply(ctx) {
  ...
  const handlers = {
    'catalog': async function (args) {
      ...
      const out = []
      for (const [scopeId, bucket] of buckets) {
        const isGlobal = scopeId === 'global'
        for (const skill of bucket.skills) {
          if (!isGlobal && globalNames.has(String(skill.name))) continue
          ...
          const row = {
            ...
            scopeId: scopeId,
            scopeLabel: isGlobal ? 'global' : scopeId,
            crossScope: !isGlobal && scopeId !== currentScopeId,
          }
