...
import { commandFirstToken, decodeServiceLogChunk, dshHome, dshManagerDir, envLinesToObject, expandEnvVars, findPidByPort, findProcessName, findShellOperator, formatServiceLogTime, isCommandInPath, isPidAlive, loadEnvFile, maskCommandLine, normalizeServicePath, probeTcpPort, quoteCmdArg, relateExternalProcess, relateExternalProcessStrong, resolveCmdEntryScript, resolveDetachedCommand, serviceLogFilePath, serviceStateKey, servicesLogDir, stopServicePid, stopServicePidGraceful, summarizeStartupFailure, terminatePid, tokenizeCommandLine, SERVICES_LOG_DIRNAME } from './service-core.js'
...
const OBSERVE_BLOCK_TEXT = '本技能正被观察。不要在本技能执行完就留痕。当整个用户任务（所有技能调用、所有步骤执行完毕，即将输出最终答复）完成时，若本次任务使用过本技能，做一次收尾留痕：①写一条观察留痕（1-3 行：本次使用中模糊/重复/缺失/误导的步骤或判据，引用 SKILL.md 具体小节；本次无发现则写 no-op 占位）；②追加到 _governance/pending-reviews.json（type=observe，status=pending，id=r-YYYYMMDD-<技能名>-NN，summary 填留痕全文，其余字段留空）；③不直接改 SKILL.md。留痕由 dsh-review 攒批审核后正式入账 _governance/skill-observations.json。没写留痕即违约——队列缺失条目就是证据。一次任务无论用过几个被观察技能，统一只写一条留痕，覆盖本次用过的全部被观察技能。'
...
/** 0.35.8：同名技能的用户文件副本候选路径（<name>/SKILL.md 约定布局）。 */
function skillCopyCandidates(name, cwd) {
  const candidates = [
    join(homedir(), '.dsh', 'skills', name, 'SKILL.md'),
    join(homedir(), '.agents', 'skills', name, 'SKILL.md'),
  ]
  if (typeof cwd === 'string' && cwd.length > 0) candidates.push(join(cwd, '.dsh', 'skills', name, 'SKILL.md'))
  return candidates
}

/**
 * 0.35.8：观察约定块的文件副本同步（跨会话/跨进程 BUG 修复）——观察状态以账本
 * （skill-observations.json）为唯一事实源，副本永远是同步层：对存在的候选副本
 * 幂等补（observing=true）/删（observing=false）约定块。这样无论哪个会话/进程的
 * first-wins 注册被哪个来源（包内 custom / ~/.agents/skills 等 user-agents 文件
 * 副本）摘下，服务到的正文都携带与账本一致的观察状态。返回实际改动路径；
 * 单文件失败只 warn 不抛（ENOENT 视为不存在跳过）。
 */
async function syncObserveBlockInCopies(name, observing, cwd) {
  const patched = []
  for (const file of skillCopyCandidates(name, cwd)) {
    try {
      ...
      console.warn('[dsh-manager] observe block sync failed for "' + file + '": ' + (error instanceof Error ? error.message : String(error)))
    }
  }
  return patched
}
...
// 0.38.0：接管从 detached-only 放宽到「强匹配外部进程」后，冷却复用同一机制。
...
const adoptAttemptAt = new Map() // serviceStateKey -> 上次接管尝试时间戳
...
function validateServiceEntry(input) {
  ...
  if (input.manageUrl !== undefined && input.manageUrl !== null) {
    manageUrl = String(input.manageUrl).trim()
    if (manageUrl.length > 300) return { error: '管理页面网址过长（最多 300 字符）' }
    if (manageUrl.length > 0 && !/^https?:\/\//i.test(manageUrl)) return { error: '管理页面网址必须是 http(s):// 开头的完整地址' }
  }
  ...
  return { entry: { name: name, cwd: cwd, command: command, args: args, env: env, port: port, autoStart: autoStart, autoRestart: autoRestart, detached: detached, healthUrl: healthUrl, envFile: envFile, startTimeoutMs: startTimeoutMs, manageUrl: manageUrl, note: note } }
}
    ...
    for (const key of Object.keys(parsed.services)) {
      ...
      for (const item of list) {
        ...
        entries.push({
          ...
          manageUrl: typeof item.manageUrl === 'string' ? item.manageUrl.trim() : '',
...
// 0.26.0 P1-3 + 0.38.0：外部进程接管回托管（写回 state，附日志标记）。
// 触发前提由调用方把关：detached 服务（状态丢失后端口在监听且命令行匹配任一规则）
// 或普通服务（强匹配 cwd/完整命令/服务名）。失败/已接管/冷却期内返回 false。
async function adoptRelatedProcess(path, entry, externalPid) {
  const key = serviceStateKey(path, entry.name)
  const now = Date.now()
  const lastTry = adoptAttemptAt.get(key) || 0
  if (now - lastTry < ADOPT_RETRY_MS) return false
  adoptAttemptAt.set(key, now)
  try {
    const state = await readServicesState()
    if (state.services[key] !== null && state.services[key] !== undefined) return false // 已有状态，无需接管
    const next = Object.assign({}, state.services)
    // detached 字段按配置写：普通服务接管后即普通托管语义（随 dsh 停止、崩溃清理照常）；
    // detached 服务保持独立运行语义（不随 dsh 停止，重启后由启动对账验证接管）。
    next[key] = { pid: externalPid, startedAt: now, detached: entry.detached === true }
    await writeServicesState(next)
    const note = entry.detached === true
      ? '----- dsh 运行中接管：进程 pid=' + externalPid + '（此后的日志持续由面板捕获）-----\n'
      : '----- dsh 已接管外部启动的进程 pid=' + externalPid + '（原在面板外启动；此后停止/重启由面板管理）-----\n'
    appendDetachedLogLine(path, entry.name, note).catch(function () {})
    return true
  } catch (error) {
    return false
  }
...
async function computeServiceStatus(path, entry, info) {
  ...
  if (running) {
    ...
    if (up) {
      ...
      if (externalPid !== null) {
        ...
        // 0.26.0 P1-3 + 0.38.0：身份确认后自动接管回托管——
        //   detached 服务：状态丢失后端口在监听且命令行匹配（任一规则，含 exe 基名弱匹配）即接管
        //   （0.25.0 的 exe 基名判定正是为 detached 重启接管服务的，必须保留）；
        //   普通服务：仅强匹配（cwd / 完整命令 / 服务名词边界）才接管，避免把同运行时的
        //   无关进程（仅 exe 基名命中）纳入管理后被误停/误杀。
        const adoptable = entry.detached === true
          ? externalRelated === true
          : relateExternalProcessStrong(entry, pinfo.commandLine) === true
        if (adoptable) {
          const adopted = await adoptRelatedProcess(path, entry, externalPid)
          if (adopted) {
  ...
  return {
    ...
    manageUrl: typeof entry.manageUrl === 'string' ? entry.manageUrl.trim() : '',
...
export function apply(ctx) {
  ...
  readRepoSettings().then(function (settings) {
    ...
    // 0.35.8：文件副本与账本同步（跨会话/跨进程修复）——包内注册 first-wins 可能被
    // ~/.agents/skills、~/.dsh/skills 等用户文件副本压过（本会话即如此），启动时按账本
    // 对全部候选副本幂等补/删约定块，保证任何会话/进程服务到的正文观察状态一致。
    for (const key of Object.keys(observations.skills)) {
      const entry = observations.skills[key]
      if (entry === undefined) continue
      syncObserveBlockInCopies(key, entry.observing === true, undefined).catch(function (error) {
        console.warn(`[dsh-manager] skill "${key}" observe copy sync failed: ${error instanceof Error ? error.message : String(error)}`)
      })
    }
  }).catch(function () {})
    ...
    const step = function () {
      ...
      void (async function () {
        ...
        try {
          ...
          gen = aiGeneration.get(item.hash) || 0
  ...
  const handlers = {
    ...
    'skillObserveSet': async function (args) {
      ...
      // 2) 插件技能：运行时重注册注入/移除约定块（即时生效）；并同步用户文件副本
      // （0.35.8：防别的会话/进程由副本 first-wins 注册导致约定块缺失）
      if (isPackaged) {
        ...
        const files = await syncObserveBlockInCopies(name, observing, view.cwd)
        return { ok: true, name: name, observing: observing, injected: 'runtime', files: files }
      }
      // 3) 文件技能：直接改 SKILL.md（追加/移除约定块）+ 同步全部候选副本
      try {
        const text = await readFile(definition.path, 'utf8')
        const nextText = observing ? injectObserveBlock(text) : removeObserveBlock(text)
        if (nextText !== text) await writeFile(definition.path, nextText, 'utf8')
        const files = await syncObserveBlockInCopies(name, observing, view.cwd)
        return { ok: true, name: name, observing: observing, injected: 'file', path: definition.path, files: files }
      } catch (error) {
    ...
    'serviceRegister': async function (args) {
      ...
      const entry = { name: name, cwd: rawPath, command: '', args: [], env: {}, port: null, autoStart: false, autoRestart: false, detached: false, healthUrl: null, envFile: '', startTimeoutMs: SERVICE_DEFAULT_START_TIMEOUT_MS, manageUrl: '', note: '' }
    ...
    'serviceExternalKill': async function (args) {
      ...
      // 0.38.0：守卫只拦「托管中的本服务自身进程」——占用端口者正是面板记录的存活 PID 时，
      // 才提示走「停止服务」（面板管得到它）；面板外手动启动（非托管）的相关进程直接放行优雅杀。
      // 修复死锁：external 状态下面板没有「停止服务」按钮，且 serviceStop 对无托管 PID 的
      // 进程是空操作——若守卫一视同仁拒绝，用户就两头都走不通（只能回命令行手动杀）。
      const state = await readServicesState()
      const key = serviceStateKey(target.pathKey, entry.name)
      const managedInfo = state.services[key]
      const managedSelf = managedInfo !== null && managedInfo !== undefined && isPidAlive(Number(managedInfo.pid)) && Number(managedInfo.pid) === pid
      if (relateExternalProcess(entry, pinfo.commandLine) === true && managedSelf) {
    ...
    'serviceAiDraft': async function (args) {
      ...
      parts.push('请输出 JSON：{"name":"服务名（英文短名）","command":"启动命令（如 node server.js）","args":["参数数组，可空"],"env":{"KEY":"VALUE"},"port":数字或 null,"healthUrl":null 或 "http://127.0.0.1:PORT/health","manageUrl":null 或 "http://127.0.0.1:PORT/admin","note":"一句话说明服务用途（可空）","envFile":null 或 ".env","startTimeoutMs":30000}')
      ...
      const draft = {
        ...
        manageUrl: typeof parsed.manageUrl === 'string' && parsed.manageUrl.trim().length > 0 ? parsed.manageUrl.trim().slice(0, 300) : '',
        note: typeof parsed.note === 'string' ? parsed.note.trim().slice(0, 300) : '',
        envFile: typeof parsed.envFile === 'string' ? parsed.envFile.trim().slice(0, 500) : '',
        startTimeoutMs: Number.isInteger(Number(parsed.startTimeoutMs)) && Number(parsed.startTimeoutMs) >= 1000 && Number(parsed.startTimeoutMs) <= 300000 ? Number(parsed.startTimeoutMs) : 30000,
      }
