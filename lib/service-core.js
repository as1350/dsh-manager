...
// 判断外部进程是否与本服务配置相关：true=像是外部启动的本服务 / false=疑似其他程序占用 / null=无法确认。
// 0.34.2 F2-2：收紧匹配——cwd 太短（盘符根/单字符目录）不参与子串匹配；服务名改为词边界匹配且至少
...
function relateExternalProcess(entry, commandLine) {
  ...
  if (cwd.length > 3 && cl.indexOf(cwd) !== -1) return true
  if (fullCmd.length > 0 && cl.indexOf(fullCmd) !== -1) return true
  if (name.length >= 5 && new RegExp('(^|[^a-z0-9])' + escapeRe(name) + '($|[^a-z0-9])').test(cl)) return true
  ...
  if (clBase.length > 0 && cmdFirst.length > 0 && clBase === cmdFirst) return true
  return false
}
...
export { commandFirstToken, decodeServiceLogChunk, dshHome, dshManagerDir, envLinesToObject, expandEnvVars, findPidByPort, findProcessName, findShellOperator, formatServiceLogTime, isCommandInPath, isPidAlive, loadEnvFile, maskCommandLine, normalizeServicePath, probeTcpPort, quoteCmdArg, relateExternalProcess, resolveCmdEntryScript, resolveDetachedCommand, serviceLogFilePath, serviceStateKey, servicesLogDir, stopServicePid, stopServicePidGraceful, summarizeStartupFailure, terminatePid, tokenizeCommandLine, SERVICES_LOG_DIRNAME }
